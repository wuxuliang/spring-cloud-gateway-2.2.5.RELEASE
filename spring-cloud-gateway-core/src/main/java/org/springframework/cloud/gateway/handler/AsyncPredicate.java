...
public interface AsyncPredicate<T> extends Function<T, Publisher<Boolean>> {
	...
	class AndAsyncPredicate<T> implements AsyncPredicate<T> {
		...
		public Publisher<Boolean> apply(T t) {
			...
			Publisher<Boolean> apply = left.apply(t);
			Mono<Boolean> from = Mono.from(apply);

			return from.flatMap(result -> {
				...
				if (!result) {
					...
					Publisher<Boolean> apply1 = right.apply(t);
					Mono<Boolean> from1 = Mono.from(apply1);
					System.out.println("===========");
					return from1;
				}
			});
