...
public class RoutePredicateHandlerMapping extends AbstractHandlerMapping {
	...
	protected Mono<Route> lookupRoute(ServerWebExchange exchange) {
		...
		Flux<Route> routeFlux = routes.concatMap(route -> {
			Mono<Route> just = Mono.just(route);
			Mono<Route> tMono = just.filterWhen(r -> {
				Map<String, Object> attributes = exchange.getAttributes();
				attributes.put(GATEWAY_PREDICATE_ROUTE_ATTR, r.getId());
				AsyncPredicate<ServerWebExchange> predicate = r.getPredicate();
				Publisher<Boolean> apply = predicate.apply(exchange);
				return apply;
			});
			return tMono;
		});

		Mono<Route> next = routeFlux.next();

		Mono<Route> map = next.map(route -> {
			if (logger.isDebugEnabled()) {
